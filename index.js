...
function combineAndReverseArrays(arr1, arr2) {
	...
	const reversarr = concatarr.reverse();
	return concatarr, reversarr;
...
function findElementIndexes(arr, element) {
	...
	const indexA = arr.indexOf(element);
	const indexZ = arr.lastIndexOf(element);
	const indexAZ = [];
	if (indexA !== -1) {
		indexAZ.push(indexA);
	}
...
function copyAndSwapElements(arr, target, start, end) {
	...
	const a = arr.copyWithin(target, start, end);
	return a;
...
function customUnshift(arr, ...elements) {
  ...
  // та циклу for, початкове значення лічильника на 1 менше ніж довжина
  // масива,
...
/**
 * Функція `customSome` перевіряє, чи хоча б один елемент масиву задовольняє задану умову 
 * за допомогою методу `some`.
 *  arr - Вхідний масив, який буде перевірятись.
 * condition - Функція-умова, яка буде застосовуватись до кожного елементу масиву.
 * Повертає: Результат перевірки.
 */
